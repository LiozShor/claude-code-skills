...
### Scroll-Driven Entrance — JS-free (Chrome 115+, Safari 26+)

The modern, zero-JavaScript alternative to the IntersectionObserver `data-reveal` path below. The browser
runs the animation on the compositor thread (smooth even on low-power devices). Use for below-fold reveals;
keep above-fold content on `.animate` classes.

```css
.scroll-reveal {
  animation: fadeInUp linear both;
  animation-timeline: view();          /* progress tied to the element's viewport visibility */
  animation-range: entry 0% entry 100%; /* play as it enters the viewport */
}

/* Always gate it — and guarantee content is visible without animation support */
@media (prefers-reduced-motion: reduce) {
  .scroll-reveal { animation: none; opacity: 1; }
}
@supports not (animation-timeline: view()) {
  .scroll-reveal { animation: none; opacity: 1; }  /* fallback: just show it */
}
```

Trade-off vs `data-reveal`: no JS and no observer wiring, but content is `opacity: 0` until the
`@supports`/reduced-motion fallback shows it — so always include those guards, and never use it above the
fold (it would flash empty in a screenshot/PNG export).
