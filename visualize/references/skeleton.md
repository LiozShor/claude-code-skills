...
### Evaluation requirements (auto-checked — keep the skeleton's defaults)

- **Exact CSS custom-property names** (no synonyms — not `--bg-primary`, not `--text-primary`):
  `--bg, --surface, --surface-hover, --border, --text, --text-secondary, --accent, --accent-secondary,
  --positive, --negative, --warning`. These exact names are required for evaluation compatibility.
- **Class-based themes only** — both `.theme-light` and `.theme-dark` defined with full property sets.
  Never `data-theme`, never `@media (prefers-color-scheme)` for the variables themselves.
- The checkers expect these to exist and work:
  - `cycleTheme()` — toggles the `html` theme class.
  - `toggleMenu()` — opens/closes the dropdown and closes on outside-click + Escape.
  - All top-level JS variables declared with `var`.
  - No horizontal overflow at 375px width.
  - At least one interaction beyond menu + theme (hover states, chart tooltips, filters, etc.).
  - `.viz-menu` present with toggle, theme, download-PNG, and print buttons.
  - Multiple semantic `<section>` elements for major content blocks.
