...
- **Heading-size minimums (EVALUATION CRITICAL):** h1 ≥ 3rem (48px), h2 ≥ 2rem (32px), h3 ≥ 1.5rem
  (24px), body = 1rem (16px). Each level must be visibly smaller than the previous, with **≥0.5rem**
  between levels. Slide decks: title h1 ≥ 3rem, content-slide titles ≥ 2.5rem.
- **Font-weight hierarchy (auto-checked):** h1 ≥ 700, h2 ≥ 600, h3 ≥ 500, body = 400.
...
**Hidden data-table fallback (Chart.js official a11y guidance).** For charts with more than a few data
points, mirror the dataset in a visually-hidden `<table>` and link it with `aria-describedby` — screen
readers read the table, sighted users see the chart:
```html
<canvas id="salesChart" role="img" aria-label="Monthly sales, line chart" aria-describedby="salesData"></canvas>
<table id="salesData" class="sr-only">
  <caption>Monthly sales 2026</caption>
  <thead><tr><th>Month</th><th>Sales</th></tr></thead>
  <tbody><tr><td>Jan</td><td>$10k</td></tr><tr><td>Feb</td><td>$20k</td></tr></tbody>
</table>
```

### Information Architecture & Layout

**AI-native information architecture:** Lead with conclusions, support with details. Place the most
important metric/insight above the fold. Use progressive revelation — show key data immediately, provide
drill-down on hover/click. Put contextual actions near the content they affect.

**Layout variation (every file must feel unique, not a template):**
- **Grid structure:** mix 1-, 2-, 3-col; use `span 2` for featured cards. Mobile-first pattern:
  ```css
  ...
  @media (max-width: 375px) { .card { padding: 16px; } .stat-value { font-size: 2rem; } }
  ```
- **Section rhythm:** alternate full-width sections, card grids, and single-focus sections.
- **Content density:** more content at smaller sizes reads as more professional than sparse content at
  large sizes. A dashboard with 8 KPI cards + 4 charts feels real; 4 cards + 2 charts feels like a demo.
- **One visual focal point** per file (hero stat, key chart, primary message) — not everything equal-weight.
- **No orphaned grid items:** if the last row strands one card, use `grid-column: span 2` or adjust columns.
- Always test at 768px and 375px — **no horizontal overflow allowed**.

**Above-fold animation restriction:** Above-fold content must NEVER use `data-reveal` (it starts at
`opacity: 0` and may flash empty) — use `.animate` classes instead. Reserve `data-reveal` for below-fold
content, max 3–4 sections.

### Minimum Sizing Rules
Elements must be large enough to read and feel substantial:
- **Timeline cards:** ≥280px wide, ≥20px padding. Distribute items evenly — if 5 items fill only 60% of
  vertical space, add content (e.g. a breakdown or metrics block) rather than leaving a large gap below.
- **Chart containers:** ≥300px height (≥360px dashboards); in grids use `flex-grow: 1` to fill space —
  300px is a floor, not a target.
- **Stat numbers:** ≥2rem (32px), bold/extrabold.
- **Card content padding:** ≥24px.
- **Section spacing:** ≥48px between major sections.
- **Slide headings:** ≥2rem, ≤6 words. **Body text:** ≥1rem, never smaller.

If content feels too small, it IS too small — err larger.

### Text Visibility Rules
Text must ALWAYS be visible — the #1 cause of broken outputs.
- Always use `var(--text)` / `var(--text-secondary)`; never a value close to the background.
- On gradient backgrounds: add `text-shadow: 0 1px 3px rgba(0,0,0,0.3)`.
- On hero slides with gradient/image backgrounds: add a dark overlay (`rgba(0,0,0,0.5)`).
- Mental test: "would this text be visible on BOTH dark (#030712) and light (#f8fafc) backgrounds?"

### Glass morphism (premium upgrade, use sparingly)
Beyond floating UI elements, glass containers can elevate a hero or primary card: `backdrop-filter:
blur(8px)`, a semi-transparent background (CSS var `--glass-opacity: 0.08`), and an elevated shadow.
Apply selectively for sophisticated layering — never to every card.
...
- ❌ **Rainbow colors** — stick to 2–3 palette colors + neutrals
- ❌ **Placeholder content** — never use "Lorem ipsum" or fake data
- ❌ **Generic/templated design** — each file should feel intentional
- ❌ **Missing menu** — every (non-poster) output needs the hamburger menu
...
See [animations.md](animations.md) for complete patterns.
